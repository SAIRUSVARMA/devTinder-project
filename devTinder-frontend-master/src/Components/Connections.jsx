...
const Connections = () => {
  ...
  const fetchConnections = async () => {
    try {
      dispatch(removeConnection()); // clear previous
      const response = await api.get("/user/connections"); // backend returns { data: [...] }
  ...
  if (!connections) return <p className="text-center mt-20">Loading...</p>;
